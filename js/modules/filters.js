...
(function(window) {
  ...
  Filters.getLinks = function() {
    try {
      ...
      Utils.error('Ошибка при получении ссылок:', error);
      return [];
    }
  };
  ...
  Filters.getTicketLink = function(ticketId) {
    ...
    try {
      ...
      Utils.error('Ошибка при получении ссылки на тикет:', error);
      return '';
    }
  };
    ...
    try {
      ...
      links.forEach((link) => {
        try {
          ...
          Utils.error('Ошибка при обработке ссылки:', error);
        }
      });
      ...
      try {
        ...
        Utils.error('Ошибка при поиске водительских прав в тексте страницы:', error);
      }
      
      // Выводим результаты в консоль
      if (result.tickets.length > 0 || result.countries.length > 0 || result.driver_licenses.length > 0) {
        Utils.log('Найдены данные:', result);
      } else {
        Utils.log('Данные не найдены');
      }
