...
// Локальный объект Utils для buttonSearch script
const Utils = {
  /**
    ...
    chrome.storage.local.get(['actionLoggingEnabled', 'actionLogs'], (result) => {
      if (result.actionLoggingEnabled) {
        ...
        const logEntry = {
          ...
          url: window.location.href || 'buttonSearch',
...
function performButtonSearch(url, tabId) {
    const overallSearchStartTime = performance.now();
    
    // ЛОГИРОВАНИЕ: Начало поиска через кнопку ПОИСК
    chrome.runtime.sendMessage({
        action: 'addActionLog',
        message: 'BUTTONSEARCH.JS: ЗАПУСК ПОИСКА ЧЕРЕЗ КНОПКУ',
        data: {
            searchURL: url,
            originTabId: tabId
        }
        ...
        const performSearch = async () => {
            chrome.scripting.executeScript({
                ...
                function: async () => {
                    const waitForTagsContainer = () => {
                        return new Promise((resolve) => {
                            const checkForContainer = () => {
                                ...
                                const tagElements = document.querySelectorAll('.DiagnosticsTagWithTopics__tagTitleName, .DiagnosticsTagWithTopics__tagTitle, .DriversDiagnosticLabel__title');
                                ...
                                const allTagSelectors = [
                                    ...
                                    '.row__col',
                                    '.DiagnosticsTagWithTopics__tagTitleName',
                                    '.DiagnosticsTagWithTopics__tagTitle',
                                    '.DriversDiagnosticLabel__title',
                                    '.DriversShowDriverDiagnosticsTags__headerText',
                                    '.DriversShowDriverDiagnosticsTags .DiagnosticsTagWithTopics__tagTitleName div',
                                ...
                                Utils.log(`Всего найдено ${totalElements} элементов по всем селекторам`);
                                const hasEnoughElements = totalElements > 10;
                                
                                if (tagsContainer1) {
                                    const elements1 = tagsContainer1.querySelectorAll('.DiagnosticsTagWithTopics__tagTitleName, .DiagnosticsTagWithTopics__tagTitle, .DriversDiagnosticLabel__title, div');
                                    container1HasContent = elements1.length > 0;
                                    Utils.log('Блок с тегами DriversShowDriverDiagnosticsTags найден! Содержит элементы:', container1HasContent);
                                }
                                
                                if (tagsContainer2) {
                                    const elements2 = tagsContainer2.querySelectorAll('.DiagnosticsTagWithTopics__tagTitleName, .DiagnosticsTagWithTopics__tagTitle, .DriversDiagnosticLabel__title, div');
                                    container2HasContent = elements2.length > 0;
                                    Utils.log('Блок с тегами DriversShowDriverDiagnosticsTags__content найден! Содержит элементы:', container2HasContent);
                                }
                                
                                if (((tagsContainer1 && container1HasContent) || (tagsContainer2 && container2HasContent)) && 
                                    (hasSpecificTagElements || hasEnoughElements)) {
                                    Utils.log(`Найдены конкретные элементы с тегами: ${tagElements.length} элементов`);
                                    setTimeout(() => {
                                        resolve({
                                    ...
                                    if ((tagsContainer1 || tagsContainer2) && !hasSpecificTagElements && !hasEnoughElements) {
                                        Utils.log('Блоки с тегами найдены, но конкретные элементы с тегами еще не загружены, ожидаем...');
                                    } else if (tagsContainer1 || tagsContainer2) {
                                        Utils.log('Блоки с тегами найдены, но они еще не содержат элементов, ожидаем...');
                                    } else {
                                        Utils.log('Ни один из блоков с тегами еще не загружен, ожидаем...');
                                    }
                                    setTimeout(checkForContainer, 30);
                                }
                            };
                        ...
                        return new Promise((resolve) => {
                            const checkForProfessionContainer = () => {
                                ...
                                if (professionContainer) {
                                    Utils.log('Блок с профессией водителя найден!');
                                    resolve(true);
                                } else {
                                    Utils.log('Блок с профессией водителя еще не загружен, ожидаем...');
                                    setTimeout(checkForProfessionContainer, 30);
                                }
                            };
                    ...
                    const waitForDriverInfoContainers = () => {
                        return new Promise((resolve) => {
                            const checkForDriverInfoContainers = () => {
                                ...
                                const rowSelectors = [
                                    '.row.row_gap_m.row_gutter_m.row_mode_flex',
                                    '.row.row_theme_amber.row_gap_m.row_gutter_m.row_mode_flex',
                                    '.row'
                                ];
                                ...
                                for (const selector of rowSelectors) {
                                    ...
                                    if (rows.length > 0) {
                                        Utils.log(`Найдено ${rows.length} строк по селектору: ${selector}`);
                                        break;
                                    }
                                }
                                
                                Utils.log(`Всего найдено ${rows.length} строк для поиска информации о водителе`);
                                
                                for (const row of rows) {
                                    const titleCol = row.querySelector('.row__col.row__col_width_x16');
                                    if (titleCol) {
                                        const titleText = titleCol.textContent.trim();
                                        Utils.log(`Найден заголовок: "${titleText}"`);
                                        
                                        if (titleText === 'Водитель является ИП') {
                                            ipContainer = row;
                                            Utils.log('Найден контейнер для ИП статуса');
                                        } else if (titleText === 'Тип партнерства') {
                                            partnershipContainer = row;
                                            Utils.log('Найден контейнер для типа партнерства');
                                        }
                                    }
                                }
                                
                                if (ipContainer && partnershipContainer) {
                                    Utils.log('Блоки с информацией о водителе найдены!');
                                    resolve({
                                        ipContainer,
                                        partnershipContainer
                                    });
                                } else {
                                    Utils.log(`Блоки с информацией о водителе еще не загружены. ИП: ${ipContainer ? 'найден' : 'не найден'}, Партнерство: ${partnershipContainer ? 'найдено' : 'не найдено'}, ожидаем...`);
                                    setTimeout(checkForDriverInfoContainers, 30);
                                }
                            };
                    ...
                    const findDriverInfo = (containers) => {
                        ...
                        if (containers.ipContainer) {
                            ...
                            let ipValueElement = containers.ipContainer.querySelector('.row__col:not(.row__col_width_x16)');
                            if (!ipValueElement) {
                                // Пробуем найти второй .row__col
                                const cols = containers.ipContainer.querySelectorAll('.row__col');
                                if (cols.length >= 2) {
                                    ipValueElement = cols[1];
                                }
                            }
                            if (ipValueElement) {
                                result.isIP = ipValueElement.textContent.trim();
                                Utils.log(`Найден ИП статус: "${result.isIP}"`);
                            } else {
                                Utils.log('Не удалось найти значение ИП статуса');
                            }
                        }

                        if (containers.partnershipContainer) {
                            // Пробуем разные способы извлечения значения
                            let partnershipValueElement = containers.partnershipContainer.querySelector('.row__col:not(.row__col_width_x16)');
                            if (!partnershipValueElement) {
                                // Пробуем найти второй .row__col
                                const cols = containers.partnershipContainer.querySelectorAll('.row__col');
                                if (cols.length >= 2) {
                                    partnershipValueElement = cols[1];
                                }
                            }
                            
                            if (partnershipValueElement) {
                                // Проверяем, есть ли ссылка внутри
                                const linkElement = partnershipValueElement.querySelector('a.g-link');
                                if (linkElement) {
                                    result.partnershipType = linkElement.textContent.trim();
                                } else {
                                    result.partnershipType = partnershipValueElement.textContent.trim();
                                }
                                Utils.log(`Найден тип партнерства: "${result.partnershipType}"`);
                            } else {
                                Utils.log('Не удалось найти значение типа партнерства');
                            }
                        }
                    ...
                    const findTags = async (customTagsPresets = []) => {
                        ...
                        return new Promise((resolve) => {
                            chrome.storage.local.get([
                                ...
                                Utils.log('Стандартные теги для поиска: auto_courier, walking_courier');
                                ...
                                const searchTagsInElements = (elements) => {
                                    Utils.log('Начинаю поиск тегов в элементах. Всего элементов:', elements.length);
                                    ...
                                    for (const element of elements) {
                                        ...
                                        if (text.includes('auto_courier') || 
                                            ...
                                            Utils.log('Найден стандартный тег: auto_courier');
                                        }
                                        ...
                                        if (text.includes('walking_courier') || 
                                            ...
                                            Utils.log('Найден стандартный тег: walking_courier');
                                        }
                                        ...
                                        for (const presetKey of customTagsPresets) {
                                            ...
                                            for (let i = 1; i <= 4; i++) {
                                                ...
                                                if (tagName && text.includes(tagName.toLowerCase())) {
                                                    tags[tagKey] = true;
                                                    Utils.log(`Найден пользовательский тег из пресета ${presetKey}: ${tagName} в тексте: "${text}"`);
                                                }
                                            }
                                        }
                                    }
                                ...
                                // Оптимизированный поиск только в конкретных контейнерах
                                const containers = window.foundTagContainers || { tagsContainer1: null, tagsContainer2: null };
                                
                                // Ищем только в контейнере DriversShowDriverDiagnosticsTags
                                if (containers.tagsContainer1) {
                                    Utils.log('Найден блок с тегами DriversShowDriverDiagnosticsTags, выполняю поиск в нем');
                                    
                                    // Ищем в названиях тегов
                                    const tagTitles = containers.tagsContainer1.querySelectorAll('.DiagnosticsTagWithTopics__tagTitleName div, .DiagnosticsTagWithTopics__tagTitle div');
                                    if (tagTitles.length > 0) {
                                        Utils.log(`Найдено ${tagTitles.length} тегов в блоке DriversShowDriverDiagnosticsTags`);
                                    ...
                                    if (tagTopics.length > 0) {
                                        Utils.log(`Найдено ${tagTopics.length} топиков в блоке DriversShowDriverDiagnosticsTags`);
                                        searchTagsInElements(tagTopics);
                                    }
                                    
                                    // Если не нашли в конкретных элементах, ищем во всех div
                                    if (!standardTags.auto_courier && !standardTags.walking_courier) {
                                        const allDivs = containers.tagsContainer1.querySelectorAll('div');
                                        if (allDivs.length > 0) {
                                            Utils.log(`Найдено ${allDivs.length} div элементов в блоке DriversShowDriverDiagnosticsTags`);
                                            searchTagsInElements(allDivs);
                                        }
                                    }
                                }
                                
                                // Ищем только в контейнере DriversShowDriverDiagnosticsTags__content (если первый не найден)
                                if (!containers.tagsContainer1 && containers.tagsContainer2) {
                                    Utils.log('Найден блок с тегами DriversShowDriverDiagnosticsTags__content, выполняю поиск в нем');
                                    
                                    // Ищем в названиях тегов
                                    const tagTitles = containers.tagsContainer2.querySelectorAll('.DiagnosticsTagWithTopics__tagTitleName div, .DiagnosticsTagWithTopics__tagTitle div');
                                    if (tagTitles.length > 0) {
                                    ...
                                    if (!standardTags.auto_courier && !standardTags.walking_courier) {
                                        ...
                                        if (allDivs.length > 0) {
                                            Utils.log(`Найдено ${allDivs.length} div элементов в блоке DriversShowDriverDiagnosticsTags__content`);
                                            searchTagsInElements(allDivs);
                                        }
                                    }
                                }
                    ...
                    return new Promise(async (resolve) => {
                        chrome.storage.local.get([
                            ...
                            try {
                                ...
                                if (presets.length === 0) {
                                    Utils.log('Все пресеты поиска отключены. Поиск тегов и профессии не будет выполнен.');
                                    
                                    resolve({
                                        success: false,
                                        message: 'Все пресеты поиска отключены'
                                    });
                                    return;
                                }
                                ...
                                Utils.log('Пресет 1:', preset1);
                                Utils.log('Пресет 2:', preset2);
                                Utils.log('Пресет 3:', preset3);
                                ...
                                Utils.log('Пресеты пользовательских тегов для поиска:', customTagsPresets);
                                
                                if (shouldSearchTags || shouldSearchCustomTags) {
                                    Utils.log('Ожидаем загрузки хотя бы одного из блоков с тегами...');
                                    ...
                                    while (attempts < maxAttempts) {
                                        attempts++;
                                        Utils.log(`Попытка ${attempts} из ${maxAttempts} найти контейнеры с тегами...`);
                                        ...
                                        if (containers.tagsContainer1) {
                                            ...
                                            if (elements.length > 5) {
                                                hasElements = true;
                                                Utils.log(`Контейнер 1 содержит ${elements.length} элементов`);
                                            }
                                        }
                                        ...
                                        if (containers.tagsContainer2) {
                                            ...
                                            if (elements.length > 5) {
                                                hasElements = true;
                                                Utils.log(`Контейнер 2 содержит ${elements.length} элементов`);
                                            }
                                        }
                                        
                                        if (hasElements) {
                                            Utils.log('Контейнеры с тегами содержат достаточное количество элементов, продолжаем поиск');
                                            break;
                                        } else {
                                            Utils.log('Контейнеры с тегами найдены, но они не содержат достаточного количества элементов, ожидаем...');
                                            await new Promise(resolve => setTimeout(resolve, 30));
                                        }
                                    }
                                    
                                    Utils.log('Хотя бы один из блоков с тегами загружен, продолжаем поиск');
                                ...
                                if (shouldSearchProfession || shouldSearchPartnershipType || shouldSearchIPStatus) {
                                    Utils.log('Ожидаем загрузки блока с профессией водителя...');
                                    await waitForProfessionContainer();
                                    Utils.log('Блок с профессией водителя загружен, продолжаем поиск');
                                    
                                    if (shouldSearchProfession) {
                                        professionResult = findProfession();
                                    }
                                    
                                    if (shouldSearchPartnershipType || shouldSearchIPStatus) {
                                        Utils.log('Ожидаем загрузки блоков с информацией о водителе...');
                                        const driverInfoContainers = await waitForDriverInfoContainers();
                                        Utils.log('Блоки с информацией о водителе загружены, продолжаем поиск');
                                ...
                                Utils.error('Ошибка при выполнении поиска:', error);
                                resolve({
                                    profession: null,
                                    driverInfo: null,
                                    tags: null,
                ...
                if (!results || !results[0] || !results[0].result) {
                    Utils.log('Нет результатов поиска');
                    return;
                }
                ...
                if (error) {
                    Utils.error('Произошла ошибка при поиске:', error);
                    setTimeout(() => {
                        chrome.tabs.remove(newTab.id);
                        Utils.log('Вкладка поиска закрыта из-за ошибки');
                    }, 200);
                    return;
                }
                    ...
                    if (settings.notifications === false) {
                        Utils.log('Уведомления отключены в настройках. Результаты поиска не будут показаны.');
                        setTimeout(() => {
                            chrome.tabs.remove(newTab.id);
                            Utils.log('Вкладка поиска закрыта, так как уведомления отключены');
                        }, 200);
                        return;
                    }
                    ...
                    if (presets.length === 0) {
                        Utils.log('Все пресеты поиска отключены. Поиск не будет выполнен.');
                        setTimeout(() => {
                            chrome.tabs.remove(newTab.id);
                            Utils.log('Вкладка поиска закрыта, так как все пресеты отключены');
                        }, 200);
                        return;
                    }
                    ...
                    for (let i = 0; i < presets.length; i++) {
                        ...
                        Utils.log(`Обработка пресета ${presetNumber}: ${preset}`);
                        ...
                        if (preset === 'profession' && profession) {
                            ...
                            // ЛОГИРОВАНИЕ: Создание уведомления профессии из buttonSearch.js
                            chrome.runtime.sendMessage({
                                action: 'addActionLog',
                                message: 'BUTTONSEARCH.JS: СОЗДАНИЕ УВЕДОМЛЕНИЯ ПРОФЕССИИ',
                                data: {
                        ...
                        if (preset === 'tags' && tags && tags.standardTags) {
                            ...
                            // ЛОГИРОВАНИЕ: Создание уведомления стандартных тегов из buttonSearch.js
                            chrome.runtime.sendMessage({
                                action: 'addActionLog',
                                message: 'BUTTONSEARCH.JS: СОЗДАНИЕ УВЕДОМЛЕНИЯ СТАНДАРТНЫХ ТЕГОВ',
                                data: {
                                    presetNumber: presetNumber,
                                    standardTagsMessage: standardTagsMessage,
                                    stack: new Error().stack.split('\n').slice(0, 5).join('\n')
                                }
                            }).catch(() => {});
                        ...
                        if (preset.startsWith('customTags') && tags && tags.customTagsResults) {
                            ...
                            if (presetResult) {
                                ...
                                // ЛОГИРОВАНИЕ: Создание уведомления пользовательских тегов из buttonSearch.js
                                chrome.runtime.sendMessage({
                                    action: 'addActionLog',
                                    message: 'BUTTONSEARCH.JS: СОЗДАНИЕ УВЕДОМЛЕНИЯ ПОЛЬЗОВАТЕЛЬСКИХ ТЕГОВ',
                                    data: {
                                        preset: preset,
                                        presetNumber: presetNumber,
                    ...
                    // ЛОГИРОВАНИЕ: Завершение поиска через кнопку ПОИСК
                    chrome.runtime.sendMessage({
                        action: 'addActionLog',
                        message: 'BUTTONSEARCH.JS: ЗАВЕРШЕНИЕ ПОИСКА ЧЕРЕЗ КНОПКУ',
                        data: {
                            searchURL: url,
                            newTabId: newTab.id,
                            presetsProcessed: presets.length,
                            processedPresets: presets
                        }
                    }).catch(() => {});
                    
                    setTimeout(() => {
                        chrome.tabs.remove(newTab.id);
                        Utils.log('Вкладка поиска закрыта после показа уведомлений');
                    }, 200);
                });
            });
        };
