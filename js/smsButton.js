...
console.log('SMS Button script загружен');
...
function enableDebugMode() {
  console.log('Включение отладочного режима...');
  ...
  XMLHttpRequest.prototype.send = function(data) {
    ...
    if (xhr._url && (
      ...
      console.log('Перехвачен запрос к API:', xhr._url);
...
const waitForIframe = (timeout = 5000) => {
  return new Promise((resolve, reject) => {
    ...
    const checkForIframe = () => {
      ...
      if (iframes.length > 0) {
        console.log(`Найдено ${iframes.length} iframe на странице`);
...
function selectRussiaOption() {
  ...
  if (russiaOptionSelected) {
    console.log('Опция Russia уже была выбрана, пропускаем поиск');
    return;
  }
  
  console.log('Ищем поле ввода и опцию Russia в выпадающем меню...');
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          if (filterInput) {
            console.log('Найдено поле ввода для фильтра:', filterInput);
            ...
            console.log('Введено значение "Russia" в поле фильтра');
            ...
            setTimeout(() => {
              ...
              if (russiaOption && russiaOption.textContent === 'Russia') {
                console.log('Найдена опция Russia:', russiaOption);
                console.log('Выбираем опцию Russia');
              ...
              console.log(`Найдено ${allOptions.length} опций в списке`);
              
              for (const option of allOptions) {
                if (option.textContent === 'Russia') {
                  console.log('Найдена опция Russia среди всех опций:', option);
                  console.log('Выбираем опцию Russia');
                  russiaOptionSelected = true;
                  option.click();
                  observer.disconnect();
        ...
        console.error('Ошибка при поиске поля ввода в iframe:', error);
      }
    }
  });
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          const filterInput = iframe.contentDocument.querySelector('.g-text-input__control.g-text-input__control_type_input.g-select-filter__input');
          if (filterInput) {
            console.log('Найдено поле ввода для фильтра напрямую:', filterInput);
            ...
            setTimeout(() => {
              ...
              const russiaOption = iframe.contentDocument.querySelector('span.g-select-list__option-default-label');
              if (russiaOption && russiaOption.textContent === 'Russia') {
                console.log('Найдена опция Russia напрямую:', russiaOption);
                console.log('Выбираем опцию Russia');
                russiaOptionSelected = true;
                russiaOption.click();
                observer.disconnect();
              ...
              // Если не найдена точная опция, попробуем найти все опции и выбрать подходящую
              const allOptions = iframe.contentDocument.querySelectorAll('span.g-select-list__option-default-label');
              console.log(`Найдено ${allOptions.length} опций в списке напрямую`);
              
              for (const option of allOptions) {
                if (option.textContent === 'Russia') {
                  console.log('Найдена опция Russia среди всех опций напрямую:', option);
                  console.log('Выбираем опцию Russia');
                  russiaOptionSelected = true;
                  option.click();
                  observer.disconnect();
        ...
        console.error('Ошибка при прямом поиске поля ввода в iframe:', error);
      }
    }
  ...
  setTimeout(() => {
    if (!russiaOptionSelected) {
      console.error('Опция Russia не найдена после 10 секунд поиска');
    }
    clearInterval(searchInterval);
    observer.disconnect();
  }, 10000);
...
function selectDeliOption() {
  ...
  if (deliOptionSelected) {
    console.log('Опция Deli уже была выбрана, пропускаем поиск');
    return;
  }
  
  console.log('Ищем селектор Intent и опцию Deli в выпадающем меню...');
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          if (intentInput) {
            console.log('Найдено поле ввода для Intent:', intentInput);
            ...
            console.log('Введено значение "Deli" в поле фильтра Intent');
            
            // Даем немного времени для обработки ввода и появления отфильтрованных результатов
            setTimeout(() => {
              // Ищем все опции в выпадающем меню
              const allOptions = iframe.contentDocument.querySelectorAll('span.g-select-list__option-default-label');
              console.log(`Найдено ${allOptions.length} опций в списке Intent`);
              
              // Ищем точное совпадение "Deli"
              for (const option of allOptions) {
                if (option.textContent === 'Deli') {
                  console.log('Найдена точная опция Deli:', option);
                  console.log('Выбираем опцию Deli');
            ...
            if (intentButton) {
              console.log('Найдена ПУСТАЯ кнопка Intent:', intentButton);
              console.log('Нажимаем на ПУСТУЮ кнопку Intent');
              intentButton.click();
            }
          }
        }
      } catch (error) {
        console.error('Ошибка при поиске селектора Intent в iframe:', error);
      }
    }
  });
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          // Если поле ввода уже открыто, вводим "Deli"
          if (intentInput) {
            console.log('Найдено поле ввода для Intent напрямую:', intentInput);
            ...
            console.log('Введено значение "Deli" в поле фильтра Intent напрямую');
            
            // Даем немного времени для обработки ввода и появления отфильтрованных результатов
            setTimeout(() => {
              // Ищем все опции в выпадающем меню
              const allOptions = iframe.contentDocument.querySelectorAll('span.g-select-list__option-default-label');
              console.log(`Найдено ${allOptions.length} опций в списке Intent напрямую`);
              
              // Ищем точное совпадение "Deli"
              for (const option of allOptions) {
                if (option.textContent === 'Deli') {
                  console.log('Найдена точная опция Deli напрямую:', option);
                  console.log('Выбираем опцию Deli');
                  deliOptionSelected = true;
                  option.click();
                  observer.disconnect();
            ...
            const intentButton = iframe.contentDocument.querySelector('button#intent[role="combobox"]:empty, button#intent[role="combobox"]:not(:has(*))');
            if (intentButton) {
              console.log('Найдена ПУСТАЯ кнопка Intent напрямую:', intentButton);
              console.log('Нажимаем на ПУСТУЮ кнопку Intent');
              intentButton.click();
        ...
        console.error('Ошибка при прямом поиске селектора Intent в iframe:', error);
      }
    }
  ...
  setTimeout(() => {
    if (!deliOptionSelected) {
      console.error('Опция Deli не найдена после 10 секунд поиска');
    }
    clearInterval(searchInterval);
    observer.disconnect();
  }, 10000);
...
function findAndClickOriginSelector() {
  ...
  if (originSelectorClicked) {
    console.log('Селектор Origin уже был нажат, пропускаем поиск');
    return;
  }
  
  console.log('Ищем селектор Origin...');
    ...
    for (const mutation of mutations) {
      if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
        ...
        for (const iframe of iframes) {
          try {
            ...
            if (iframe.contentDocument) {
              ...
              if (originButton) {
                console.log('MutationObserver: Найден пустой селектор Origin:', originButton);
                console.log('Нажимаем на селектор Origin');
        ...
        if (!originSelectorClicked) {
          ...
          if (originButton) {
            console.log('MutationObserver: Найден пустой селектор Origin в основном документе:', originButton);
            console.log('Нажимаем на селектор Origin');
            originSelectorClicked = true;
            originButton.click();
  ...
  function directSearchForOrigin() {
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          if (originButton) {
            console.log('Найден пустой селектор Origin напрямую:', originButton);
            console.log('Нажимаем на селектор Origin');
            originSelectorClicked = true;
            originButton.click();
          ...
          for (const control of originControls) {
            ...
            if (button && button.children.length === 0) {
              console.log('Найден селектор Origin через родительский контейнер напрямую:', button);
              console.log('Нажимаем на селектор Origin');
              originSelectorClicked = true;
              button.click();
    ...
    if (!originSelectorClicked) {
      const originButton = document.querySelector('button#origin:not(:has(*))');
      if (originButton) {
        console.log('Найден пустой селектор Origin в основном документе напрямую:', originButton);
        console.log('Нажимаем на селектор Origin');
        originSelectorClicked = true;
        originButton.click();
    ...
    if (!originSelectorClicked) {
      ...
      for (const control of originControls) {
        ...
        const button = control.querySelector('button#origin');
        if (button && button.children.length === 0) {
          console.log('Найден селектор Origin через родительский контейнер в основном документе напрямую:', button);
          console.log('Нажимаем на селектор Origin');
          originSelectorClicked = true;
          button.click();
  ...
  timeoutId = setTimeout(() => {
    ...
    observer.disconnect();
    if (!originSelectorClicked) {
      console.error('Селектор Origin не найден после 10 секунд поиска');
    }
  }, 10000);
}
...
function fillTicketField() {
  ...
  if (ticketInputFilled) {
    console.log('Поле с ID тикета уже заполнено, пропускаем поиск');
    return;
  }
  
  console.log('Ищем поле ввода для ID тикета...');
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          if (ticketInput) {
            console.log('Найдено поле ввода для ID тикета:', ticketInput);
            ...
            chrome.storage.local.get(['smsTicketId'], (result) => {
              ...
              console.log('Получен ID тикета из хранилища:', ticketId);
              ...
              console.log('Ссылка на тикет вставлена в поле:', ticketLink);
        ...
        console.error('Ошибка при поиске поля ввода для ID тикета в iframe:', error);
      }
    }
  });
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          const ticketInput = iframe.contentDocument.querySelector('input[name="ticket"], input.g-text-input__control[name="ticket"]');
          if (ticketInput) {
            console.log('Найдено поле ввода для ID тикета напрямую:', ticketInput);
        ...
        console.error('Ошибка при прямом поиске поля ввода для ID тикета в iframe:', error);
      }
    }
  ...
  setTimeout(() => {
    if (!ticketInputFilled) {
      console.error('Поле ввода для ID тикета не найдено после 10 секунд поиска');
    }
    clearInterval(searchInterval);
    observer.disconnect();
  }, 10000);
...
function fillMessageText() {
  ...
  if (messageTextFilled) {
    console.log('Текстовое поле сообщения уже заполнено, пропускаем поиск');
    return;
  }
  
  console.log('Ищем текстовое поле для сообщения...');
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          if (messageTextarea) {
            console.log('Найдено текстовое поле для сообщения:', messageTextarea);
            ...
            chrome.storage.local.get(['smsType', 'smsTexts'], (result) => {
              ...
              console.log('Получен тип SMS из хранилища:', smsType);
              ...
              console.log('Текст сообщения вставлен в поле:', messageText);
              ...
              chrome.storage.local.remove('smsType', () => {
                console.log('Тип SMS удален из хранилища для предотвращения автоматического заполнения в будущем');
              });
            });
          }
        }
      } catch (error) {
        console.error('Ошибка при поиске текстового поля для сообщения в iframe:', error);
      }
    }
  });
    ...
    for (const iframe of iframes) {
      try {
        ...
        if (iframe.contentDocument) {
          ...
          const messageTextarea = iframe.contentDocument.querySelector('textarea.g-text-area__control[name="payload.text"]');
          if (messageTextarea) {
            console.log('Найдено текстовое поле для сообщения напрямую:', messageTextarea);
        ...
        console.error('Ошибка при прямом поиске текстового поля для сообщения в iframe:', error);
      }
    }
  ...
  setTimeout(() => {
    if (!messageTextFilled) {
      console.error('Текстовое поле для сообщения не найдено после 10 секунд поиска');
    }
    clearInterval(searchInterval);
    observer.disconnect();
  }, 10000);
...
function findMultipleElements() {
  console.log('Запускаем ускоренный поиск всех элементов формы...');
    ...
    try {
      if (iframe.contentDocument) {
        ...
        if (!newMailingButtonClicked) {
          ...
          for (const button of buttons) {
            if (button.innerText && button.innerText.includes('Новая рассылка')) {
              console.log('Ускоренный поиск: Найдена кнопка "Новая рассылка"');
              newMailingButtonClicked = true;
              button.click();
              break;
            }
            ...
            for (const span of spans) {
              if (span.innerText && span.innerText.includes('Новая рассылка')) {
                console.log('Ускоренный поиск: Найдена кнопка с вложенным текстом "Новая рассылка"');
                newMailingButtonClicked = true;
                button.click();
                break;
              }
        ...
        if (newMailingButtonClicked && !originSelectorClicked) {
          ...
          for (const label of labels) {
            if (label.innerText && label.innerText.includes('Origin')) {
              console.log('Ускоренный поиск: Найден селектор Origin');
        ...
        if (originSelectorClicked && !russiaOptionSelected) {
          ...
          for (const option of options) {
            if (option.textContent === 'Russia') {
              console.log('Ускоренный поиск: Найдена опция Russia');
              russiaOptionSelected = true;
              option.click();
              break;
            }
        ...
        if (russiaOptionSelected && !deliOptionSelected) {
          ...
          for (const option of options) {
            if (option.textContent === 'Deli') {
              console.log('Ускоренный поиск: Найдена опция Deli');
              deliOptionSelected = true;
              option.click();
              break;
            }
        ...
        if (deliOptionSelected && !ticketInputFilled) {
          ...
          if (inputs.length > 0) {
            console.log('Ускоренный поиск: Найдено поле ввода для ID тикета');
            ...
            chrome.storage.local.get(['smsTicketId'], (result) => {
              ...
              console.log('Заполнено поле ID тикета:', ticketId);
            });
          }
        }
        ...
        if (ticketInputFilled && !messageTextFilled) {
          ...
          if (textareas.length > 0) {
            console.log('Ускоренный поиск: Найдено текстовое поле для сообщения');
            ...
            chrome.storage.local.get(['smsType', 'smsTexts'], (result) => {
              ...
              console.log('Заполнено текстовое поле сообщения для типа:', smsType);
      ...
      console.error('Ошибка при одновременном поиске элементов в iframe:', error);
    }
  }
}
...
function logMessage(type, message) {
  chrome.storage.local.get(['loggingEnabled'], (result) => {
    if (result.loggingEnabled) {
      ...
      console.log(`[${timestamp}] [SMS-ОТПРАВКА] [${typeText}] ${message}`);
    }
  });
}
...
chrome.storage.local.get(['smsType'], (result) => {
  if (result.smsType) {
    ...
    console.log('Страница была открыта напрямую без нажатия кнопки расширения. Автозаполнение отключено.');
  }
}); 