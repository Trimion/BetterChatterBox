...
function performSearchAndNotify() {
  ...
  chrome.storage.local.get([
    ...
    const NOTIFICATION_DELAY = 270; // Уменьшаем задержку до 270 миллисекунд (уменьшено на 10%)
      ...
      try {
        ...
        notificationTimer = setTimeout(() => {
          try {
              ...
              try {
                ...
                chrome.runtime.sendMessage({ 
                  ...
                  if (chrome.runtime.lastError) {
                    ...
                    Utils.log('Получен ответ с ID уведомления:', response.notificationId);
                    currentNotificationId = response.notificationId;
                  } else if (response && response.error) {
                    Utils.log('Получена ошибка от background.js:', response.error);
                  } else {
                    Utils.log('Получен ответ без ID уведомления:', response);
                  }
                });
                Utils.log('Сообщение отправлено в background.js');
              } catch (error) {
                Utils.log('Исключение при отправке сообщения:', error);
              }
            ...
            Utils.log('Ошибка при отправке уведомления:', error);
          }
        }, NOTIFICATION_DELAY);
      } catch (error) {
        Utils.log('Ошибка при сборе данных:', error);
      }
    }
      ...
      try {
        ...
        if (valuesChanged) {
          ...
          if (ticketId && ticketId.ticketId !== lastTicketId) {
            ...
            // Отправляем данные для создания уведомления
            collectData(values);
          }
        }
      } catch (error) {
    ...
    const observer = new MutationObserver((mutations) => {
      try {
        ...
        // Устанавливаем новый таймаут на 300 миллисекунд (уменьшаем задержку)
        ...
        }, 300); // Уменьшаем задержку до 300 миллисекунд
      } catch (error) {
